...
# Team ID:			[ 1211 ]
# Author List:		[ Ayushman, aniket, Piyush, Prachi ]
# Filename:			task_2a.py
# Functions:		
# Global variables:	
...
def transform_vision_sensor_depth_image(vision_sensor_depth_image, image_resolution):
	...
	##############	ADD YOUR CODE HERE	##############
	transformed_depth_image = np.resize(np.array(vision_sensor_depth_image,dtype=np.float32), (image_resolution[0],image_resolution[1]))
...
def detect_berries(transformed_image, transformed_depth_image):
	...
	##############	ADD YOUR CODE HERE	##############
	h_min,h_max=[165,100,25],[175,135,35]
	for i,berry in enumerate(berries):
		berries_dictionary[berry],lower,upper=[],np.array([h_min[i],170,170]),np.array([h_max[i],255,255])
		hsv=cv2.cvtColor(transformed_image,cv2.COLOR_BGR2HSV)
		mask=cv2.inRange(hsv,lower,upper)
		cont,hier=cv2.findContours(mask,cv2.RETR_LIST,cv2.CHAIN_APPROX_NONE)
		for cnt in cont:
			...
			berries_dictionary[berry].append((cX,cY,transformed_depth_image[cY,cX]+(w*0.5*scale-0.01)/(2-0.01)))
	##################################################
...
def detect_berry_positions(berries_dictionary):
	...
	for berry in berries:
		berry_positions_dictionary[berry]=[]
		for i in berries_dictionary[berry]:
			berry_positions_dictionary[berry].append(((i[0]-256)*scale,(i[1]-256)*scale,(2-0.01)*i[2]+0.01))
	##################################################
