...
Original file is located at
    https://colab.research.google.com/drive/1uL6Q0DwLHlP6zVrLN7H1yxKi6Zw4jRHx

# Task 1B: 
focus on the functions given before main. The main part is not to be changed.

remote api functions info: https://coppeliarobotics.com/helpFiles/en/remoteApiFunctionsPython.htm
<br> remote api constants: https://coppeliarobotics.com/helpFiles/en/remoteApiConstants.htm#operationModes
"""
...
def get_vision_sensor_image(client_id):
	...
	return_code, vis_sensor_handle = sim.simxGetObjectHandle(client_id, 'vision_sensor', sim.simx_opmode_blocking)
	return_code, image_resolution, vision_sensor_image = sim.simxGetVisionSensorImage(client_id, vis_sensor_handle, 0, sim.simx_opmode_streaming) # simx_opmode_streaming(the first call) and simx_opmode_buffer (the following calls)

	while(1): 
		return_code, image_resolution, vision_sensor_image = sim.simxGetVisionSensorImage(client_id, vis_sensor_handle, 0, sim.simx_opmode_buffer) # simx_opmode_streaming(the first call) and simx_opmode_buffer (the following calls)
		if return_code == 0:
			break
...
def detect_qr_codes(transformed_image):
	...
	for qrcode in qrcodeoriginal:
		...
		qrlist.append(qrcode.data.decode("utf-8"))
		qrlist.append(centroid)
		qr_codes.append(qrlist)
